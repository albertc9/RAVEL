...
reg [511:0] inputs[0:2351];
reg [31:0] expected[0:97];
integer stalls=0, gaps=0, completed=0;
reuse2_heldout dut(.ap_clk(clk), .ap_rst_n(rst), .ap_start(start), .ap_ready(control_ready),
 .heldout_input_TDATA(in_data), .heldout_input_TVALID(in_valid), .heldout_input_TREADY(in_ready),
 .layer11_out_TDATA(out_data), .layer11_out_TVALID(out_valid), .layer11_out_TREADY(out_ready));
task reset_dut;
begin
...
begin
 ...
 while(sent < samples*24 || received < samples || launched < samples) begin
   @(negedge clk);
   if(!pending && sent < samples*24 && cycles%7 != 0) begin
   ...
   if(!in_valid && sent < samples*24) gaps=gaps+1;
