...
reg [127:0] inputs[0:9471];
reg [31:0] expected[0:73];
integer stalls=0, gaps=0, completed=0;
legacy_p2d2 dut(.ap_clk(clk), .ap_rst_n(rst), .ap_start(start), .ap_ready(control_ready),
 .waveform_TDATA(in_data), .waveform_TVALID(in_valid), .waveform_TREADY(in_ready),
 .layer9_out_TDATA(out_data), .layer9_out_TVALID(out_valid), .layer9_out_TREADY(out_ready));
task reset_dut;
begin
...
begin
 ...
 while(sent < samples*128 || received < samples || launched < samples) begin
   @(negedge clk);
   if(!pending && sent < samples*128 && cycles%7 != 0) begin
   ...
   if(!in_valid && sent < samples*128) gaps=gaps+1;
