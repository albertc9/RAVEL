...
reg [63:0] inputs[0:2943];
reg [31:0] expected[0:91];
integer stalls=0, gaps=0, completed=0;
two_block_c3 dut(.ap_clk(clk), .ap_rst_n(rst), .ap_start(start), .ap_ready(control_ready),
 .renamed_input_TDATA(in_data), .renamed_input_TVALID(in_valid), .renamed_input_TREADY(in_ready),
