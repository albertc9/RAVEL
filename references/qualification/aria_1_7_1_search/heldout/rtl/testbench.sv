...
reg [95:0] inputs[0:3679];
reg [31:0] expected[0:91];
integer stalls=0, gaps=0, completed=0;
heldout dut(.ap_clk(clk), .ap_rst_n(rst), .ap_start(start), .ap_ready(control_ready),
...
begin
 ...
 while(sent < samples*40 || received < samples || launched < samples) begin
   @(negedge clk);
   if(!pending && sent < samples*40 && cycles%7 != 0) begin
   ...
   if(!in_valid && sent < samples*40) gaps=gaps+1;
