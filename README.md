...
### Like-for-like HLS and OOC comparison

All four flows below use the exact same Keras model, hls4ml configuration, part, and
clock target. The vanilla project is emitted directly by hls4ml without manual
changes to generated C++, headers, Tcl, or YAML.

| Flow | II | Latency (cycles) | Est. clock (ns) |
|---|---:|---:|---:|
| Vanilla hls4ml | 3076 | 3084 | 3.619 |
| RAVEL Aria 1.1.0 P2/D1 | 178 | 183 | 3.647 |
| RAVEL Aria 1.3.0 P4/D2 | 94 | 99 | 3.502 |
| RAVEL Aria 1.4.0 P4/D2 | 94 | 99 | 3.402 |

Single-core resources below are Vivado 2023.2 post-route results for the
generated `cnn_core`, implemented out of context on `xcku5p-ffvb676-2-e` with
a 5 ns clock constraint. They are physical implementation counts, not Vitis
HLS estimates or full-system utilization.

| Flow | BRAM tile | DSP | CLB registers | CLB LUTs | WNS (ns) |
|---|---:|---:|---:|---:|---:|
| Vanilla hls4ml | 5.5 | 1 | 21745 | 18312 | +0.206 |
| RAVEL Aria 1.1.0 P2/D1 | 1.0 | 7 | 2953 | 5332 | +1.121 |
| RAVEL Aria 1.3.0 P4/D2 | 0.5 | 14 | 3709 | 7861 | +0.817 |
| RAVEL Aria 1.4.0 P4/D2 | 1.5 | 15 | 3622 | 6981 | +0.524 |

All four designs are fully routed with no routing errors and meet the 200 MHz
target. Aria 1.4 retains the Aria 1.3 II and latency while reducing post-route
CLB LUTs by 11.2% and CLB registers by 2.3%. Its sequential packed Dense weight
ROM increases BRAM use from 0.5 to 1.5 tiles; DSP use increases from 14 to 15.
Supporting HLS evidence is in the [Aria 1.4 synthesis and RTL CoSim
report](references/cnn_for_arianna/reports/aria_1_4_p4d2.json)
